...
const App: FC = () => {
    ...
    useEffect(() => {
        const ensureUsageAnalytics = async () => {
            // Wait for initial data to be fetched
            await new Promise(resolve => setTimeout(resolve, 500));
    
            setFolders(currentFolders => {
                const usageFolderExists = currentFolders.some(f => f.name === 'Usage Analytics');
                if (!usageFolderExists) {
                    const newFolder = { id: 'f-usage-analytics', name: 'Usage Analytics', color: '#8E44AD' };
            ...
            setDashboards(currentDashboards => {
                const designDashboardExists = currentDashboards.some(d => d.id === 'd-fusion-design');
                const viewerDashboardExists = currentDashboards.some(d => d.id === 'd-fusion-viewer');
                ...
                if (dashboardsToAdd.length > 0) {
                    return [...currentDashboards, ...dashboardsToAdd];
                }
        ...
        ensureUsageAnalytics();
    }, []);

    const applyTheming = useCallback((options: any, formatter: (this: any) => string, gridLineStyle: GridlineStyle) => {
    ...
    const ltdSpendOnBeforeRender = useCallback((options: any, gridLineStyle: GridlineStyle) => {
    ...
    const actualForecastOnBeforeRender = useCallback((options: any, gridLineStyle: GridlineStyle) => {
        options.chart = { ...options.chart, alignTicks: true };
        options.plotOptions = { ...options.plotOptions, column: { ...options.plotOptions?.column, borderRadius: 1, crisp: false, groupPadding: 0.4 } };
    ...
    const handleUpdateDashboardFolder = async (dashboardId: string, folderId: string | null) => {
        setDashboards(prevDashboards =>
            prevDashboards.map(d =>
    ...
    const currentDashboardName = dashboards.find(d => d.id === activeDashboardId)?.name || (isCreatingNewDashboard ? 'New Dashboard' : 'FP&A Demo');
                ...
                <div className="app-root">
                    ...
                    <div className="app-body">
                        <SidePanel 
                            ...
                            onAllDashboardsClick={() => {setShowAllDashboards(true); setActiveDashboardId(null); setActiveView(null);}}
                                ...
                                <>
                                    ...
                                    >
                                        {widgets.map((w) => {
                                            ...
                                            const isEmbed = w.id === 'embed' || w.id === 'styled-embed';

                                            return (
                                                <div
                                                    key={w.instanceId}
                                                    className={`widget-container ${isEditable ? 'is-editable' : ''} ${isEmbed ? 'is-embed' : ''}`}
                                                    onContextMenu={(e) => handleContextMenu(e, w.instanceId)}
                                                >
                                                    {w.id === 'styled-embed' && w.widgetOid && w.dashboardOid ? (
                                                        <DashboardWidget
                                                            widgetOid={w.widgetOid}
                                                            dashboardOid={w.dashboardOid}
                                                            styleOptions={w.styleConfig ? getStyleOptionsFromConfig(w.styleConfig) : getStyleOptions(theme)}
                                                            onBeforeRender={w.styleConfig ? (options: any) => onBeforeRenderStyledWidget(options, w.styleConfig) : undefined}
                                                        />
                                                    ) : w.id === 'embed' && w.embedCode ? (
                                                      <CodeBlock code={w.embedCode} styleOptions={getStyleOptions(theme)} />
                                                    ) : WidgetComponent ? (
                                                        <WidgetComponent
                                                            {...oids}
                                                            themeMode={theme}
                                                        ...
                                                        <DashboardWidget
                                                            ...
                                                            onBeforeRender={
                                                                ...
                                                                (w.id === 'table1' || w.id === 'chart6') ? (options: any) => applyTheming(options, detailedBudgetTooltipFormatter, gridLineStyle) :
