...
export interface SidePanelProps {
  ...
  onUpdateDashboardFolder: (dashboardId: string, folderId: string | null) => void;
...
const SidePanel: React.FC<SidePanelProps> = ({
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    if (isDashboard) {
      const newFolderId = destination.droppableId === 'root-dashboards' ? null : destination.droppableId;
  ...
  const filteredFolders = folders.filter(folder =>
    ...
    dashboards.some(d => d.folderId === folder.id && d.name && d.name.toLowerCase().includes(searchTerm.toLowerCase()))
  );
                    ...
                    {(provided) => (
                        <ul {...provided.droppableProps} ref={provided.innerRef}>
                            ...
                            {filteredFolders.map((folder) => (
                                <li key={folder.id} className={`${openFolders[folder.id] || searchTerm ? 'open' : ''} folder-item`}>
                                    <div onContextMenu={(e) => handleContextMenu(e, folder.id, 'folder')}>
